...
import { CancellationToken, ExtensionContext, NotebookDocument, TextDocument, Uri } from 'vscode';
...
export function isJupyterNotebook(document: NotebookDocument) {
    return document.viewType === JupyterNotebookView;
}
export function isJupyterCell(document: TextDocument) {
    return document.notebook && isJupyterNotebook(document.notebook);
}
